...
- **HomeKit services** (all defined in `accessory.ts`, each with a fixed
  ...
  | Switch (momentary, generic "Stop Script") | `script-stop` | `GET /scripts/stop` (on press) |
...
- **Script buttons.** `setupScriptButtons()` (called once from the accessory
  constructor) creates a generic momentary `Switch` "Stop Script" (subtype
  `script-stop` → `GET /scripts/stop`), then does an async `GET /scripts/list`
  and creates one **momentary `Switch`** per script (subtype `script-<id>`,
  named after the script). Flipping a script switch on calls
  `POST /scripts/execute/` and then `setTimeout`s the switch back to off —
  `onGet` always returns `false`. Handlers are re-wired on every launch (they
  don't survive a cache restore). If `/scripts/list` is unreachable at startup,
  cached script switches are kept (and re-wired) rather than deleted. The whole
  group (including the stop button) is toggled by config `exposeScripts`
  (default `true`). The plugin can't *create* scripts — the Xenia API has no
  such endpoint; users author them on the machine.
- **Stale-service cleanup.** When `waterTankSensor` config changes, when an
  ...
  when a `script-*` switch no longer matches a script on the machine (or
...
`GET /scripts/list`, `GET /scripts/stop`, `POST /machine/control/`,
`POST /toggle_sb`, `POST /inc_dec`, `POST /inc_dec_bb`,
`POST /scripts/execute/`. See <https://www.xenia-espresso.de/api.html>.
